...
function menu_waterfall {
    while true; do
        ...
        printf "${yellow}01) 1.11\n"
        printf "${yellow}02) 1.12\n"
        printf "${yellow}03) 1.13\n"
        printf "${yellow}04) 1.14\n"
        printf "${yellow}05) 1.15\n"
        printf "${yellow}06) 1.16\n"
        printf "${yellow}07) 1.17\n"
        printf "${yellow}08) 1.18\n"
        printf "${yellow}09) 1.19\n"
        printf "${yellow}10) 1.20\n"
        printf "${yellow}11) 1.21\n"
        printf "${blue}00) Phiên bản đã cài đặt${reset}\n"
        printf "${blue}0) Quay lại${reset}\n"
        read -r choice
        case $choice in
            1) install_waterfall 1.11 ;;
            2) install_waterfall 1.12 ;;
            3) install_waterfall 1.13 ;;
            4) install_waterfall 1.14 ;;
            5) install_waterfall 1.15 ;;
            6) install_waterfall 1.16 ;;
            7) install_waterfall 1.17 ;;
            8) install_waterfall 1.18 ;;
            9) install_waterfall 1.19 ;;
            10) install_waterfall 1.20 ;;
            11) install_waterfall 1.21 ;;
            00) subdirectory_waterfall ;;
            0) return ;;
        esac
    done
}
...
function install_waterfall {
    local version=$1
    local url=${waterfall_urls[$version]}
    ...
    printf "Xác nhận cài đặt? ${yellow}[y/n]${reset}\n"
        ...
        if [[ "$confirm" =~ [yY] ]]; then
            while true; do
                ...
                sleep 1
                local main_directory="Waterfall"
                ...
                if [[ -d "$main_directory/$subdirectory" ]]; then
                    ...
                    while true; do
                        ...
                        if [[ "$overwrite" =~ [yY] ]]; then
                            ...
                            if wget "$url" -O waterfall.jar; then
                                                    ...
                                                    while true; do
                                                        ...
                                                        case $option in
                                                            1)
                                                                ...
                                                                java -Xms128M -XX:MaxRAMPercentage=95.0 -jar waterfall.jar
                                                                ;;
                                                            2)
                                                                clear
                                                                printf "${green}Đang khởi động Proxy Server...${reset}\n"
                                                                java -Xms128M -XX:MaxRAMPercentage=95.0 -XX:+UseG1GC -XX:+ParallelRefProcEnabled -XX:MaxGCPauseMillis=200 -XX:+UnlockExperimentalVMOptions -XX:+DisableExplicitGC -XX:+AlwaysPreTouch -XX:G1HeapWastePercent=5 -XX:G1MixedGCCountTarget=4 -XX:InitiatingHeapOccupancyPercent=15 -XX:G1MixedGCLiveThresholdPercent=90 -XX:G1RSetUpdatingPauseTimePercent=5 -XX:SurvivorRatio=32 -XX:+PerfDisableSharedMem -XX:MaxTenuringThreshold=1 -Dusing.aikars.flags=https://mcflags.emc.gs -Daikars.new.flags=true -XX:G1NewSizePercent=30 -XX:G1MaxNewSizePercent=40 -XX:G1HeapRegionSize=8M -XX:G1ReservePercent=20  -Dterminal.jline=false -Dterminal.ansi=true -jar waterfall.jar
                                                                ;;
                                                        esac
                                                        break 2
                                                    done
                                                ...
                                                2)
                                                    ...
                                                    while true; do
                                                        ...
                                                        case $option in
                                                            1)
                                                                printf "${green}Đang khởi động Proxy Server...${reset}\n"
                                                                screen -dmS "Proxy-Waterfall--$version" bash -c "java -Xms128M -XX:MaxRAMPercentage=95.0 -jar waterfall.jar"
                                                                printf "${green}Proxy-Waterfall--$version đã khởi động ${reset}\n"
                                                                ;;
                                                            2)
                                                                printf "${green}Đang khởi động Proxy Server...${reset}\n"
                                                                screen -dmS "Proxy-Waterfall--$version" bash -c "java -Xms128M -XX:MaxRAMPercentage=95.0 -XX:+UseG1GC -XX:+ParallelRefProcEnabled -XX:MaxGCPauseMillis=200 -XX:+UnlockExperimentalVMOptions -XX:+DisableExplicitGC -XX:+AlwaysPreTouch -XX:G1HeapWastePercent=5 -XX:G1MixedGCCountTarget=4 -XX:InitiatingHeapOccupancyPercent=15 -XX:G1MixedGCLiveThresholdPercent=90 -XX:G1RSetUpdatingPauseTimePercent=5 -XX:SurvivorRatio=32 -XX:+PerfDisableSharedMem -XX:MaxTenuringThreshold=1 -Dusing.aikars.flags=https://mcflags.emc.gs -Daikars.new.flags=true -XX:G1NewSizePercent=30 -XX:G1MaxNewSizePercent=40 -XX:G1HeapRegionSize=8M -XX:G1ReservePercent=20  -Dterminal.jline=false -Dterminal.ansi=true -jar waterfall.jar"
                                                                printf "${green}Proxy-Waterfall--$version đã khởi động ${reset}\n"
                                                                ;;
                                                        esac
                                                        break 2
                                ...
                                printf "${red}Không thể tải về Waterfall Proxy. Vui lòng thử lại sau${reset}\n"
                                sleep 0.5
...
function subdirectory_waterfall {
    while true; do
        clear
        display_menu
        printf "${yellow}=============================   Menu Waterfall   ============================${reset}\n"
        ...
        if [[ -d "Waterfall" ]]; then
            ...
            for subdirectory in "Waterfall"/*; do
            ...
            if [[ -n "$selected_version" && -d "Waterfall/$selected_version" ]]; then
                printf "\n"
                ...
                while true; do
                    ...
                    case $action in
                        1)
                            cd "Waterfall/$selected_version" || exit
                            printf "${green}Bạn muốn chạy đơn hay đa Proxy Server?${reset}\n"
                            printf "${yellow}1) Đơn${reset}\n"
                            printf "${yellow}2) Đa Server${reset}\n"
                            printf "\n"
                        ...
                        2)
                            ...
                            while true; do
                                ...
                                if [[ "$confirm_delete" =~ [yY] ]]; then
                                    rm -rf "Waterfall/$selected_version"
