...
function menu_folia {
    while true; do
        clear
        display_menu
        printf "${yellow}=========================   Menu Minecraft Folia   ==========================\n"
        printf "\n"
        printf "${green}Chọn phiên bản bạn muốn cài đặt: \n"
        printf "${yellow}1) 1.19.4\n"
        printf "${yellow}2) 1.20.1\n"
        printf "${yellow}3) 1.20.2\n"
        printf "${yellow}4) 1.20.4\n"
        printf "${yellow}5) 1.20.6\n"
        printf "${yellow}6) 1.21.4\n"
        ...
        read -r choice
        case $choice in
            1) install_folia_version 1.19.4 ;;
            2) install_folia_version 1.20.1 ;;
            3) install_folia_version 1.20.2 ;;
            4) install_folia_version 1.20.4 ;;
            5) install_folia_version 1.20.6 ;;
            6) install_folia_version 1.21.4 ;;
            00) subdirectory_folia ;;
            0) return ;;
...
function install_folia_version {
    local version=$1
    local url=${folia_urls[$version]}
    ...
    printf "Xác nhận cài đặt? ${yellow}[y/n]${reset}\n"
        ...
        if [[ "$confirm" =~ [yY] ]]; then
            while true; do
                ...
                sleep 1
                local main_directory="Folia Server"
                    ...
                    while true; do
                        ...
                        if [[ "$overwrite" =~ [yY] ]]; then
                            ...
                            if wget "$url" -O folia.jar; then
                                                    ...
                                                    while true; do
                                                        ...
                                                        case $option in
                                                            1)
                                                                ...
                                                                printf "${green}Đang khởi động Server...${reset}\n"
                                                                screen -dmS "Folia--$version" bash -c "java -Xms128M -XX:MaxRAMPercentage=95.0 -jar pufferfish.jar --nogui"
                                                                printf "${green}Folia--$version đã khởi động trên cổng: ${yellow}$port${reset}\n"
                                                                ;;
                                                            2)
                                                                if [[ -f "$config_file" ]]; then
                                                                    sed -i "s/^server-port=.*/server-port=$port/" "$config_file"
                                                                ...
                                                                printf "${green}Đang khởi động Server...${reset}\n"
                                                                screen -dmS "Folia--$version" bash -c "java -Xms128M -XX:MaxRAMPercentage=95.0 -XX:+UseG1GC -XX:+ParallelRefProcEnabled -XX:MaxGCPauseMillis=200 -XX:+UnlockExperimentalVMOptions -XX:+DisableExplicitGC -XX:+AlwaysPreTouch -XX:G1HeapWastePercent=5 -XX:G1MixedGCCountTarget=4 -XX:InitiatingHeapOccupancyPercent=15 -XX:G1MixedGCLiveThresholdPercent=90 -XX:G1RSetUpdatingPauseTimePercent=5 -XX:SurvivorRatio=32 -XX:+PerfDisableSharedMem -XX:MaxTenuringThreshold=1 -Dusing.aikars.flags=https://mcflags.emc.gs -Daikars.new.flags=true -XX:G1NewSizePercent=30 -XX:G1MaxNewSizePercent=40 -XX:G1HeapRegionSize=8M -XX:G1ReservePercent=20  -Dterminal.jline=false -Dterminal.ansi=true -jar folia.jar --nogui"
                                                                printf "${green}Folia--$version đã khởi động trên cổng: ${yellow}$port${reset}\n"
                                                                ;;
                                                        esac
                                                        break 2
                                ...
                                printf "${red}Không thể tải về Minecraft Folia. Vui lòng thử lại sau${reset}\n"
                                sleep 0.5
...
function subdirectory_folia {
    while true; do
        ...
        if [[ -d "Folia Server" ]]; then
            count=1
            declare -A version_map
            for subdirectory in "Folia Server"/*; do
            ...
            if [[ -n "$selected_version" && -d "Folia Server/$selected_version" ]]; then
                printf "\n"
                ...
                while true; do
                    ...
                    case $action in
                        1)
                            cd "Folia Server/$selected_version" || exit
                            printf "${green}Bạn muốn chạy đơn hay đa Server?${reset}\n"
                            printf "${yellow}1) Đơn${reset}\n"
                            printf "${yellow}2) Đa Server${reset}\n"
                            printf "\n"
                        ...
                        while true; do
                            ...
                            if [[ "$confirm_delete" =~ [yY] ]]; then
                                rm -rf "Folia Server/$selected_version"
